...
//    }
    ...
    public function resubmitRequest($transactionNumber)
    {
        ...
        $isApproverChange = $this->isApproverListChanged($approverIds, $assetApprovalIds);
//        if($isApproverChange){
//            return 'inside';
//        }
//        return 'outside';
    ...
    public function isApproverChange($transactionNumber, bool $isUpdated)
    {
        $user = auth('sanctum')->user();
        $subUnitId = $this->getSubUnitId($transactionNumber);
        $approverIds = $this->getApproverIds($subUnitId);
...
//            $this->updateAssetRequestStatus($assetRequest->transaction_number, 'Returned' . ($assetApproval->layer));
    ...
    {
        return DepartmentUnitApprovers::where('subunit_id', $subUnitId)->orderBy('layer')->get()->pluck('approver_id')->toArray();
    }
