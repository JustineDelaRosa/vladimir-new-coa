...
//                    $addedUsefulLife = $addedUsefulLife / 12;
                ...
                ->filter(function ($fixed_asset) {
                    ...
                    return $startDepreciationDate->lessThanOrEqualTo(now()->startOfMonth()) //->startOfMonth()
                        && $endDepreciationDate->greaterThanOrEqualTo(now()->startOfMonth()); //->startOfMonth()
                })
                ->values();
...
//        $items = [];
            ...
            foreach ($selectedAsset as $fixed_asset) {
                ...
                $remainingBookValue = $depreciationHistory->last()->book_value ?? $formula->depreciable_basis;

                if ($fixed_asset->depreciationStatus->depreciation_status_name !== 'Fully Depreciated' && $fixed_asset->additionalCost->isEmpty()) {
                    $remainingBookValue = $formula->depreciable_basis;
                }
                ...
                $goodAddCostTotal = $goodAdditionalCost->sum('depreciable_basis') ?? 0;
                $depreciationValue = $goodAddCostTotal + $remainingBookValue;
                $totalDepreciationValue = $formula->depreciable_basis + $additionalCost->sum('depreciable_basis');
                ...
                $monthsDepreciated = 0; //$this->calculationRepository->getMonthDifference($formula->start_depreciation, now()->format('Y-m')) ?? 0;
                ...
                $accumulated_cost = $this->calculationRepository->getAccumulatedCost($monthly_depreciation, $actualMonthsDepreciated, $depreciationValue) ?? 0;
...
//            }
                ...
                $previousAccumulatedCost = $lastDepreciationHistory ? $lastDepreciationHistory->accumulated_cost : $accumulated_cost;
                ...
                $currentRemainingBookValue = ($previousRemainingBookValue - $remainingBookValue) - $monthly_depreciation;
...
//            $yearsDepreciated = floor(($monthsDepreciated + $actualMonthsDepreciated) / 12);
                ...
                if ($fixed_asset->depreciation_method === "One Time" || ($fixed_asset->depreciation_method === "One Time" && $additionalCost->count() > 0)) {
                    $currentRemainingBookValue = $totalDepreciationValue;
                    if ($lastDepreciationHistory) {
                        $currentRemainingBookValue = $totalDepreciationValue - $lastDepreciationHistory->accumulated_cost;
                    }

                    if ($currentRemainingBookValue != 0) {
                        $currentRemainingBookValue = 0;
                        $depreciationValue = $additionalCost->sum('depreciable_basis');
                        $currentAccumulatedCost = $totalDepreciationValue;
                    }
                    ...
                    DepreciationHistory::create([
                        'fixed_asset_id' => $fixed_asset->id,
                        'depreciated_date' => now()->format('Y-m'),
                        'depreciation_per_month' => $depreciationValue + $totalDepreciationValue,
                        'months_depreciated' => $monthsDepreciated + $actualMonthsDepreciated,
                        'depreciation_per_year' => 0,
                        'accumulated_cost' => $currentAccumulatedCost,
                        'remaining_book_value' => $currentRemainingBookValue,
                    ...
                    if ($currentRemainingBookValue < 1) {
...
//        return $items;
            ...
            $this->info("Monthly depreciation set successfully: {$selectedAsset} date: " . now()->format('Y-m'));
        } catch (\Exception $e) {
