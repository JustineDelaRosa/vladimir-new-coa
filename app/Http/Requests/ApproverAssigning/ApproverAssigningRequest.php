...
namespace App\Http\Requests\ApproverAssigning;

use App\Models\UserApprover;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class ApproverAssigningRequest extends FormRequest
{
    /**
        ...
        if ($this->isMethod("POST")) {
            return [
                ...
                'approver_id' => [
                    ...
                    'array',
                    'unique_in_array',
                    //check if the requester already has the approver check all values in the array
                    function ($attribute, $value, $fail) {
                        foreach ($value as $approver) {
                            $userApprover = UserApprover::where('requester_id', $this->requester_id)->where('approver_id', $approver)->first();
                            if ($userApprover) {
